...
<script>
    new DataTable('#parceirosBuscar', {
        processing: true,
        responsive: true,
        scrollX: true,
        ajax: {
            url: '{{ route("parceirosData") }}',
            dataSrc: 'data'
        },
        language: {
            ...
            },
            {
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton${row.id}">
                        <a class="dropdown-item" href="${row.parceirosEdit}">Editar - ${row.nome}</a>
                        <a class="dropdown-item delete-btn" href="#" data-url="${row.parceirosExclud}" data-id="${row.id}">Deletar - ${row.nome}</a>
                    </div>
                </div>
                `;
                },
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            title: 'Deletado!',
                            text: 'O parceiro foi excluída com sucesso.',
                            icon: 'success',
                            backdrop: true, // Impede o fechamento ao clicar fora
                            allowEscapeKey: false, // Não permite fechar com ESC
                            allowOutsideClick: false // Não permite fechar ao clicar fora
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            title: 'Erro!',
                            text: 'Houve um erro ao tentar excluir o parceiro. Tente novamente mais tarde.',
                            icon: 'error',
                            backdrop: true, // Impede o fechamento ao clicar fora
                            allowEscapeKey: false, // Não permite fechar com ESC
                            allowOutsideClick: false // Não permite fechar ao clicar fora
